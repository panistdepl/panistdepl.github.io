...
var firefoxLinkAPI = 'download/panist-' + firefoxVersion + '-an+fx-api-panit.xpi';
...
if (bowser.firefox) {
  if (bowser.check({ firefox: "49" })) {
    $('#addingToBrowserAPI').text('Ajouter l\'extension à Firefox');
    $('#addingToBrowserAPI').attr('href', firefoxLinkAPI);
    ...
    $('#addingToBrowserAPI').text('Version du navigateur ancienne.');
    ...
    $('#addingToBrowserAPI').attr("disabled", true);
    $('#addingToBrowser').attr("disabled", true);
  }
} else {
  // Not supported
  $('#addingToBrowserAPI').text('Navigateur non supporté.');
